...
/**
 * The glyph the legend prints for a kind. Exported so a surface that marks up individual changes shows the
 * same symbol its legend explains — a private copy in a feature would be free to drift, and a legend that
 * explains a glyph nothing renders is worse than none.
 */
export function cpDiffGlyph(kind: CpDiffKind): string { return CP_DIFF_LEGEND_GLYPHS[kind]; }
/**
 * The legend's own wording for a kind. Exported for the same reason as {@link cpDiffGlyph}, and needed for a
 * second reason: the glyph carries meaning by color and shape, so the text beside it is what makes a change
 * readable to a screen reader and in a high-contrast mode (WCAG 2.2 AA, 1.4.1).
 */
export function cpDiffLabel(kind: CpDiffKind): string { return CP_DIFF_LEGEND_DEFAULT_LABELS[kind]; }
@Component({
  ...
  glyphFor(kind: CpDiffKind): string { return cpDiffGlyph(kind); }
  labelFor(kind: CpDiffKind): string { return this.labels()[kind] ?? cpDiffLabel(kind); }
}
