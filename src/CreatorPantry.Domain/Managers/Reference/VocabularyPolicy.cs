...
/// <c>Code</c> first, then a <see cref="Data.VocabularyAlias"/>, then a <see cref="NormalizeAlias"/>
/// comparison against the display name. The last step is deliberately not a stored column — these
/// vocabularies hold tens of rows, not the hundreds of thousands <see cref="Data.Ingredient"/> anticipates,
...
public static class VocabularyPolicy
{
    ...
    /// <summary>Matches <see cref="DisplayNameMaxLength"/>: an alias is another way of writing the same name.</summary>
    public const int AliasMaxLength = DisplayNameMaxLength;
    ...
    /// <remarks>
    /// <para>
    /// Deliberately the same rule as <see cref="IngredientPolicy.NormalizeName"/> and deliberately
    /// <em>not</em> <see cref="MeasurementPolicy.NormalizeAlias"/>. Unit codes are short and closed, so
    ...
    /// then need rebuilding, the same way <see cref="Data.Ingredient.SearchText"/> would.
    /// </para>
    /// </remarks>
    public static string NormalizeAlias(string value) => IngredientPolicy.NormalizeName(value);
}
